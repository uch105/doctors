...
def additional(request,pk):
    if request.method == "POST":
        ...
        if len(doctors) == 0:
            ...
            SelectedTheme.objects.create(doctor=doctor,theme=theme)
...
def signin(request):
    ...
    if context['Validation']:
        try:
            ...
            instance = Doctor.objects.create(bmdc=c+bmdc,category=category,sub_category=sub_category,bsub_category=sub_category.bangla,name=context['name'],reg_year=context['regyear'],valid_till=context['regvalidyear'],blood_group=context['bg'],status= True,dob=context['dob'],fname=context['fname'],mname=context['mname'])
            instance.save()
...
def dashboard_createprescription(request):
    ...
    if request.method == "POST":
        ...
        template_image_path = config('TEMPLATE_IMAGE_PATH')+SelectedTheme.objects.filter(doctor=doctor)[0].theme.image.url
        output_pdf_path = config("OUTPUT_PDF_PATH")+f'{pid}.pdf'

        if SelectedTheme.objects.filter(doctor=doctor)[0].theme.name == "Default":
            create_pdf(template_image_path=template_image_path,output_pdf_path=output_pdf_path,cc=cc,rf=rf,oe=oe,ix=ix,dx=dx,prescription_text=prestext,chamber=doctor.chamber,dr_name_e=doctor.name,dr_d_e=doctor.qualification,dr_name_b=doctor.bname,dr_d_b=doctor.bqualification,p_name=fullname,p_age=age,p_sex=sex,p_address=address)

        with open(output_pdf_path,'rb') as pdf_file:
            patient = Patient(doctor=doctor,pid=pid,name=fullname,age=age,sex=sex,address=address,contact=contact,pdf=f"files/prescription/{pid}.pdf")
            patient.save()
...
@login_required(login_url="/log-in/")
def dashboard_previewprescription(request):
    doctor = Doctor.objects.get(bmdc=get_username(request))
    if request.method == "POST":
        fullname = request.POST.get("fullname")
        age = request.POST.get("age")
        ...
        template_image_path = config('TEMPLATE_IMAGE_PATH')+SelectedTheme.objects.filter(doctor=doctor)[0].theme.image.url
        output_pdf_path = config("TEMP_OUTPUT_PDF_PATH")+f'{pid}.pdf'

        create_pdf(template_image_path=template_image_path,output_pdf_path=output_pdf_path,cc=cc,rf=rf,oe=oe,ix=ix,dx=dx,prescription_text=prestext,chamber=doctor.chamber,dr_name_e=doctor.name,dr_d_e=doctor.qualification,dr_name_b=doctor.bname,dr_d_b=doctor.bqualification,p_name=fullname,p_age=age,p_sex=sex,p_address=address)

        with open(output_pdf_path,'rb') as pdf_file:
            patient = TempPatientData(doctor=doctor,file=f"files/tmp/{pid}.pdf")
            patient.save()

        return JsonResponse({
            'pdf_url': patient.file.url,
        })
...
@login_required(login_url="/log-in/")
def dashboard_editprescription(request,pk):
    doctor = Doctor.objects.get(bmdc=get_username(request))
    if request.method == "POST":
        fullname = request.POST.get("fullname")
        age = request.POST.get("age")
        ...
        with open(output_pdf_path,'rb') as pdf_file:
            patient = Patient.objects.filter(doctor=doctor,pid=pid)[0]
        ...
        PatientDetail.objects.filter(patient=patient,symptom="Chief Complaints").update(details=cc)
        PatientDetail.objects.filter(patient=patient,symptom="Risk Factors").update(details=rf)
        PatientDetail.objects.filter(patient=patient,symptom="On Examination").update(details=oe)
        PatientDetail.objects.filter(patient=patient,symptom="Diagnosis").update(details=dx)
        PatientDetail.objects.filter(patient=patient,symptom="Investigations").update(details=ix)
        PatientDetail.objects.filter(patient=patient,symptom="Prescriptions").update(details=prestext)
        
        return redirect('createprescription')
    
    patient = Patient.objects.filter(doctor=doctor,pid=pk)[0]
    cc_symptom = PatientDetail.objects.filter(patient=patient,symptom="Chief Complaints")[0]
    rf_symptom = PatientDetail.objects.filter(patient=patient,symptom="Risk Factors")[0]
    oe_symptom = PatientDetail.objects.filter(patient=patient,symptom="On Examination")[0]
    ...
    context = {
        ...
        'oe_symptom': oe_symptom,
        'dx_symptom': dx_symptom,
        'ix_symptom': ix_symptom,
        'prestext_symptom': prestext_symptom,
        'sub_valid': Subscription.objects.get(doctor=doctor).is_active,
    }
    return render(request,"dapp/dashboard-prescription-edit.html",context)
...
def oeapi(request):
    if request.method == "get" or request.method == "GET":
        ...
        oes = list(OE.objects.filter(text__icontains=query).annotate(text_length=Length('text')).order_by('text_length', '-priority_index').values())
        return JsonResponse(oes,safe=False)
    
@login_required(login_url="/log-in/")
def ixapi(request):
    if request.method == "get" or request.method == "GET":
        query = request.GET.get('query')
        ixs = list(Ix.objects.filter(text__icontains=query).annotate(text_length=Length('text')).order_by('text_length', '-priority_index').values())
        return JsonResponse(ixs,safe=False)

@login_required(login_url="/log-in/")
def ccapi(request):
    if request.method == "get" or request.method == "GET":
        query = request.GET.get('query')
        ccs = list(CC.objects.filter(text__icontains=query).annotate(text_length=Length('text')).order_by('text_length', '-priority_index').values())
        return JsonResponse(ccs,safe=False)
    
@login_required(login_url="/log-in/")
def rfapi(request):
    if request.method == "get" or request.method == "GET":
        query = request.GET.get('query')
        rfs = list(RF.objects.filter(text__icontains=query).annotate(text_length=Length('text')).order_by('text_length', '-priority_index').values())
        return JsonResponse(rfs,safe=False)

@login_required(login_url="/log-in/")
def dxapi(request):
    if request.method == "get" or request.method == "GET":
        query = request.GET.get('query')
        dxs = list(Dx.objects.filter(text__icontains=query).annotate(text_length=Length('text')).order_by('text_length', '-priority_index').values())
        return JsonResponse(dxs,safe=False)

@login_required(login_url="/log-in/")
def adviceapi(request):
    if request.method == "get" or request.method == "GET":
        query = request.GET.get('query')
        advices = list(AdviceTemplate.objects.filter(doctor=Doctor.objects.get(bmdc=get_username(request)),index=query).values())
        return JsonResponse(advices,safe=False)
...
@login_required(login_url="/log-in/")
def dashboard_setupprofile(request):
    doctor = Doctor.objects.get(bmdc=get_username(request))
    if request.method == "POST":
        doctor.chamber = request.POST.get("chamber")
        doctor.name = request.POST.get("dr_name_e")
        doctor.qualification = request.POST.get("dr_d_e")
        doctor.bname = request.POST.get("dr_name_b")
        doctor.bqualification = request.POST.get("dr_d_b")
        doctor.save()
    ...
    return render(request,"dapp/dashboard-setup-profile.html",context)
